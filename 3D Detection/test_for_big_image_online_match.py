...
if __name__ == '__main__':
    ...
    MaskPath = './ribbon_crop_cell2_final'
    ...
    length = 256
    label_start = 1
    for start in range(0, len(files), length-4):
        end = min(length+start, len(files))
        mask_z_path = os.path.join(MaskPath, str(start)+'-'+str(end))
        if not os.path.exists(mask_z_path):
            os.mkdir(mask_z_path)
        img = []
        cells = []
        ...
        for tt in range(out_x.shape[0]):
            cv2.imwrite(mask_z_path + '\\' + str(start + tt).zfill(4) + '.png', out_x[tt])
        # if start == 0:
        #     out_all = out_x
        # else:
        #     out_all = pair_match(out_all, out_x, direction=1, halo_size=4)

    # for zz in range(out_all.shape[0]):
    #     cv2.imwrite(MaskPath + '\\' + str(zz).zfill(4) + '.png', out_all[zz])